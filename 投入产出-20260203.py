...
# 参数设置
BASE_SAFETY_SHARE = 0.4267
SLIDER_CAP = 1.0
SLIDER_STEP = 0.001
HOME_BG_FILENAME = "home_bg_4.png"  # 首页背景图（放在脚本同目录下）
...
def set_cn_font_for_matplotlib() -> str:
    """
    强制 Matplotlib 全局使用项目根目录的 SimHei.ttf
    返回实际注册到 Matplotlib 的字体名（family name）
    """
    font_path = Path(__file__).resolve().parent / "SimHei.ttf"

    if not font_path.exists():
        raise FileNotFoundError(f"找不到字体文件：{font_path}（请确认 SimHei.ttf 在脚本同级目录/仓库根目录）")
...
def inject_global_css():
    ...
    try:
        _font_name = set_cn_font_for_matplotlib()
    except Exception as e:
        st.warning(f"中文字体加载失败：{e}")
...
# Step 1：测总额（对数回归）
@st.cache_data
def load_step1_data_and_fit(file_bytes: bytes, selected_area: str) -> Tuple[pd.DataFrame, float, float]:
    df = _read_excel_from_bytes(file_bytes)
    start_row, end_row = AREA_ROW_MAP[selected_area]
...
def render_step1(file_bytes: Optional[bytes]):
    ...
    if (not selected_area) or (selected_area not in AREA_ROW_MAP):
        selected_area = st.session_state.get("last_valid_area", "")

    if (not selected_area) or (selected_area not in AREA_ROW_MAP):
        st.warning("请先在侧边栏选择有效的分析地区！")
        return

    try:
        data, a, b = load_step1_data_and_fit(file_bytes, selected_area)
    except Exception as e:
    ...
    min_target = float(reliability_2024)
    # default_target = max(float(st.session_state.get("step1_target_Y", min_target) or min_target), min_target)
    ...
    with col1:
        ...
        target_Y = st.number_input(
            ...
            format="%.4f",
            key="step1_target_Y",
            help=f"下限为【{selected_area}】2024年供电可靠率：{min_target:.4f}%，上限为100%。"
        )
        ...
        if target_Y is not None:
            ...
            with m2:
                st.metric(
                    "供电可靠率预测值",
                    f"{target_Y:.4f} %",
                    delta=f"较2024年提升幅度 {reliability_diff:.4f}%",
                    delta_color="normal",
                )
            st.subheader(f"{selected_area}历史数据（2021-2024年）")
            ...
            st.dataframe(
                ...
                column_config={
                    ...
                    "投资金额": st.column_config.NumberColumn("累计投资金额（万元）"),
    ...
    with col2:
        if target_Y is None:
            ...
            ax.scatter(data["投资金额"], data["供电可靠率"], s=120, label="历史数据", zorder=5, linewidth=2, color='#008000')
...
@st.cache_data
def fit_log_models(file_bytes: bytes, selected_area: str):
    df = _read_excel_from_bytes(file_bytes)
    start_row, end_row = AREA_ROW_MAP[selected_area]
...
def render_step2(file_bytes: Optional[bytes]):
    ...
    try:
        curve_sub, curve_main = fit_log_models(file_bytes, selected_area)
    except Exception as e:
    ...
    with col2:
        ...
        st.markdown("<div style='margin-top: 83px;'></div>", unsafe_allow_html=True)
        st.subheader(f"{selected_area}预算分配金额建议")
...
def render_home():
    ...
    if b64:
        st.markdown(
            ...
            # f'  <div class="home-title">投入产出评价<br>模型演示</div>'
            # f'</div>',
              f'<div class="home-slide" style="background-image:url(\'data:image/png;base64,{b64}\');'
              f'background-position: center center; background-repeat: no-repeat;">'
              # f'background-position: center center; background-repeat: no-repeat;background-size:100% 100%;">'
              # f'  <div class="home-title" style="text-align:center;line-height:1.6;">投入产出评价<br>模型演示</div>'
              f'  <div class="home-title" style="text-align:center;line-height:1.6; margin-top: -60px;">投入产出评价<br>模型演示</div>'  # 仅加-10px上外边距实现轻微上移
              f'</div>',
            unsafe_allow_html=True
        )
    else:
        st.markdown(
            ...
            # '  <div class="home-title">投入产出评价<br>模型演示</div>'
            # '</div>',
              '<div class="home-slide" style="background:#ffffff;border:2px solid #52c41a;">'
              '  <div class="home-title" style="text-align:center;line-height:1.6;">投入产出评价<br>模型演示</div>'
              '</div>',
            unsafe_allow_html=True
        )
...
def render_sidebar(page: str):
    with st.sidebar:
        ...
        def _on_excel_change():
            up = st.session_state.get("excel_uploader")  # 注意：读的是 session_state
            if up is None:
                # 用户点了 X 清除文件 -> 重置
                ...
                st.session_state["last_valid_area"] = ""
                st.session_state["selected_area_ui"] = "请选择分析地区"
        ...
        if st.session_state.get("excel_bytes") is not None:
            ...
            options = ["请选择分析地区"] + list(AREA_ROW_MAP.keys())

            ui_val = st.session_state.get("selected_area_ui", "请选择分析地区")
            if ui_val not in options:
                ui_val = "请选择分析地区"
                st.session_state["selected_area_ui"] = ui_val

            chosen = st.selectbox(
                "选择分析地区",
                options=options,
                index=options.index(ui_val),
                key="selected_area_ui",
            )

            if chosen != "请选择分析地区":
                st.session_state["selected_area"] = chosen
                st.session_state["last_valid_area"] = chosen
...
def main():
    ...
    st.session_state.setdefault("selected_area", "")
    st.session_state.setdefault("last_valid_area", "")
    st.session_state.setdefault("selected_area_ui", "请选择分析地区")
